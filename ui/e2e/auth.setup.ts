...
setup('authenticate via Azure AD', async ({ page }) => {
  ...
  // Navigate to the app — MSAL will redirect to Azure AD login
  await page.goto('/');

  // Azure AD login flow — enter email
  ...
  // Wait for redirect back to the app (may land on / or /dashboard)
  await page.waitForURL(/app\.pontifex\.localhost/, { timeout: 30_000 });

  // Wait for the page to fully render (authenticated content visible)
